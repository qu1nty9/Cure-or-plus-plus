...
def main() -> int:
    parser = argparse.ArgumentParser(description="Build hosted-provider VLM comparison tables.")
    parser.add_argument("--provider-matrix", default="configs/vlm_frontier_provider_matrix_v01.json")
    parser.add_argument("--csv", default="reports/vlm_provider_full_v01_comparison.csv")
    parser.add_argument("--markdown", default="reports/vlm_provider_full_v01_comparison.md")
    parser.add_argument("--latex", default="reports/vlm_provider_full_v01_comparison.tex")
    args = parser.parse_args()

    matrix = json.loads(resolve_path(args.provider_matrix).read_text(encoding="utf-8"))
    rows = build_rows(matrix)
    if not rows:
        raise RuntimeError("No completed hosted-provider VLM result directories found.")
...
def build_rows(matrix: dict) -> list[dict[str, str]]:
    ...
    for model in matrix.get("models", []):
        result_dir_text = model.get("known_v01_full_result_dir", "")
...
def build_model_row(model: dict, result_dir: Path) -> dict[str, str]:
    ...
    row = {
        "slug": model["slug"],
        "display_name": model.get("display_name") or model.get("model_id", model["slug"]),
...
def write_markdown(path: Path, rows: list[dict[str, str]], matrix: dict) -> None:
    ...
    lines = [
        ...
        f"Prompt pack: `{matrix.get('prompt_pack_path', 'reports/vlm_api_track_v01_prompt_pack.jsonl')}`.",
        ...
        "## Interpretation",
        "",
        f"The current hosted-provider leader is `{leader['display_name']}` with real-transfer accuracy `{fmt4(leader['real_accuracy'])}`.",
...
def write_latex(path: Path, rows: list[dict[str, str]]) -> None:
    lines = [
        r"\begin{table}[t]",
        r"\centering",
        r"\caption{Hosted-provider VLM full v0.1 real-transfer benchmark.}",
        r"\label{tab:vlm-provider-full-v01}",
