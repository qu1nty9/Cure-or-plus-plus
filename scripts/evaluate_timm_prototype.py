...
def main() -> int:
    parser = argparse.ArgumentParser(description="Evaluate a timm backbone with a clean-train prototype classifier.")
    parser.add_argument("--config", default="configs/hgnetv2_b0_prototype_v01.json")
    ...
    model = timm.create_model(config["model_id"], pretrained=True, num_classes=0).to(device)
    model.eval()
    ...
    centroids = build_centroids(train_features, train_labels, labels, label_to_idx, torch, F).to(device)
...
def build_centroids(features, train_labels, labels, label_to_idx, torch, F):
    centroids = []
    for label in labels:
